...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):#这个函数是最重要的函数
        ...
        if not self.outOfPixmap(pos) and  self.start and not self.started and self.task_mode==0:
            self.xuxian = pos
            self.repaint()
    ...
    def point_all_delete(self):
        ...
        self.point_cover = {i: 2 for i in range(len(self.point_link))}
        self.point_point_list=[]
        self.point_dex=0
