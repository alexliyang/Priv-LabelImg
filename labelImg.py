...
###################这部分代码开始加入整个布局了#####################
        ...
        next_rects=action('&Next Rect', self.openNextrects,
                             '+','next-rect',u'Next Point')
    ...
    def loadLabels(self, shapes):#只对检测和分割有用 ,下载标注
        s = []
        id=0
        if self.task_mode in [0,1,4]:#检测或者分割模式
            ...
            if self.task_mode==4:
                ...
                for i,r in enumerate(rects):
                    ...
                    self.canvas.draw_point_rect(rects[i][0+i*4][0],rects[i][0+i*4][1],w,h)
            if s:
    ...
    def saveLabels(self, filename):
        ...
        if self.task_mode in [0,1]:# seg and det mode
            ...
            # if self.canvas.point_rects_index == len(self.canvas.point_rects) - 1 and len(
            #         self.canvas.point_rects) > 1 :  # 为了解决最后一个框，保存键无法保存的问题and not self.points_rect_end
            #     with open(savefilename_txt, 'r') as f:
            #         lines=f.readlines()
            #         print()
            #     if int(lines[-2][-2])<=len(self.canvas.point_rects)-1:
            #         with codecs.open(savefilename_txt, 'a') as outfile:
            #             self.point_list_tmp = self.canvas.point_point_list
            #             for l in range(len(self.point_list_tmp)):
            #                 if isinstance(self.point_list_tmp[l], QPointF):  # 绘制过程
            #                     if self.point_list_tmp[l]:
            #                         outfile.write(
            #                             self.labelHist[l] + ' ' + str(self.point_list_tmp[l]).split('PyQt5.QtCore.QPointF')[
            #                                 1] + ' ' + str(True) + ' ' + str(
            #                                 self.canvas.point_rects_index) + '\n')  # str(self.canvas.point_visible[l])
            #                     else:
            #                         outfile.write(self.labelHist[l] + ' ' +
            #                                       '(0,0)' + ' ' + str(False) + ' ' + str(
            #                             self.canvas.point_rects_index) + '\n')  # str(self.canvas.point_visible[i])
            #                 else:  # 加载过程
            #                     if self.point_list_tmp[l]:
            #                         outfile.write(
            #                             self.labelHist[l] + ' ' + str(self.point_list_tmp[l]).replace('[', '(').replace(']',
            #                                                                                                             ')') + ' ' + str(
            #                                 True) + ' ' + str(
            #                                 self.canvas.point_rects_index) + '\n')  # str(self.canvas.point_visible[l])
            #                     else:
            ...
            #                                       '(0,0)' + ' ' + str(False) + ' ' + str(
            #                             self.canvas.point_rects_index) + '\n')  # str(self.canvas.point_visible[i])
            #             # self.points_rect_end=True
            # else:
            #     with codecs.open(savefilename_txt, 'w') as outfile:
            #         #版本一
            #         # for i in range(len(self.canvas.point_point_list)) :
            #         #     if self.canvas.point_point_list[i]:
            #         #         outfile.write(self.labelHist[i] + ' '+str(self.canvas.point_point_list[i]).split('PyQt5.QtCore.QPointF')[1]+' '+str(self.canvas.point_visible[i])+ '\n')#.split('PyQt5.QtCore.QPointF')[1]
            #         #     else:
            #         #         outfile.write(self.labelHist[i] + ' ' +
            #         #                       '(0,0)' + ' ' + str(self.canvas.point_visible[i]) + '\n')
            #
            #         #以下保存的是link
            #         # for i in range(len(self.canvas.point_link_save)):
            #         #     outfile.write(str(self.canvas.point_link_save[i])+'\n')
            #
            #         # for i in range(len(self.canvas.point_shape)):
            #         #     self.point_list_tmp.append(self.canvas.point_shape[i])
            #         #版本二
            #
            #         for i in range(len(self.canvas.point_shape)):
            ...
            #                         if self.point_list_tmp[l]:
            #                             outfile.write(
            #                                 self.labelHist[l] + ' ' + str(self.point_list_tmp[l]).replace('[','(').replace(']',')')+ ' ' + str(True) + ' '+str(i)+'\n')#str(self.canvas.point_visible[l])
            #                         else:
            #                             outfile.write(self.labelHist[l] + ' ' +
            #                                           '(0,0)' + ' ' + str(False)+ ' '+str(i)+ '\n')#str(self.canvas.point_visible[i])
            #             else:
    ...
    def loadFile(self, filename=None):
        ...
        if QFile.exists(filename):
            ...
            if self.task_mode in [0,1]:
                ...
                self.canvas.set_shape_type(0)
                txtPath2 = self.loadingfilepath()
                txtPath = txtPath2 + '\\' + basename + '_point.txt'
    ...
    def rects_vis(self,vis_list):
        ...
        #                 self.shapesToItems[self.labelHist.index(label)].setCheckState(Qt.Checked)
        #                 if re.search(rr, line):
        #                     x = re.search(rr, line).group(2).split(',')[0]
        #                     y = re.search(rr, line).group(2).split(',')[1].split(')')[0].strip()
        #                     point_list.append([int(float(x)), int(float(y))])
        #                 if int(line[-2]) not in self.canvas.point_shape:
        #                     if len(point_list)>1:
        #                         print(point_list[-1])
        #                         point_list=[point_list[-1]]
        #                         self.canvas.point_shape[int(line[-2])] = point_list
        #                     else:
        #                         self.canvas.point_shape[int(line[-2])] = point_list
        #                 else:
        #                     self.canvas.point_shape[int(line[-2])]=point_list[:]
        #
        #         if 0 in self.canvas.point_shape:
        #             self.canvas.point_load(self.canvas.point_shape[0])
        # else:
        #     self.canvas.point_load(False)
